...
import { Wrench, Plus, RefreshCw, UserCheck, Search, X } from 'lucide-react';

const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000/api';

export default function HomeServiceHelpers() {
  ...
  const fetchHelpers = async () => {
    try {
      ...
      if (data.success) {
        ...
        const hsHelpers = data.helpers.filter(h => h.category === 'HOME_SERVICES' || h.category === 'BOTH');
        setHelpers(hsHelpers);
      }
      setLoading(false);
    } catch (err) {
      console.error('Failed to fetch home service helpers:', err);
  ...
  if (loading) return <div className="card"><p>Loading Home Service helpers...</p></div>;
    ...
    <div>
      ...
      <div className="page-header-container">
        <div className="page-title-group">
          <h3><Wrench size={26} color="var(--primary)" /> Home Service Helpers</h3>
          <p>Manage specialized technician, electrician, plumber, and cleaning helpers.</p>
        </div>
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <span className="badge task">{filteredHelpers.length} Home Service Technicians</span>
        </div>
          ...
          <tbody>
            ...
            {filteredHelpers.map(helper => (
              <tr key={helper.id}>
                ...
                <td>
                  <select 
                    value={helper.category || 'HOME_SERVICES'} 
        ...
        <div className="modal-overlay">
          <div className="modal-card animate-fade">
            ...
            <p style={{ marginBottom: '0.5rem' }}>
              <strong>Status:</strong> <span className={`badge ${selectedHelper.status === 'ONLINE' ? 'active' : 'pending'}`}>{selectedHelper.status}</span>
            </p>
          </div>
        </div>
      )}
      ...
      {showAddModal && (
        <div className="modal-overlay">
          <div className="modal-card animate-fade">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid var(--border)', paddingBottom: '0.5rem' }}>
               <h3 style={{ margin: 0 }}>Add New Home Service Helper</h3>
